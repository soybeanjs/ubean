...
| `.server.vue` / `.client.vue` 文件约定                                                                                                                            | Server Components (Task 9,P1.5):`.server.vue` 仅 SSR 渲染(客户端不发送 JS);`.client.vue` SSR 渲染 `<!--client-only-->` 注释占位符(注释而非元素:元素在 `<tr>`/`<ul>` 等上下文会被解析器提出容器),客户端 `onMounted` 后替换为真实组件。Vite 插件自动处理 `resolveId`/`load`/`transform`,用户无需手动调用 |
