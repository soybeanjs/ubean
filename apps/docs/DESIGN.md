...
**Decision:** Ship both locales. Use ubean's vue-i18n 11 integration (`useI18n` / `setLocale` from `ubean/client`). Strategy: `prefix_except_default`. EN is default (unprefixed); zh-CN at `/zh/*`.
...
**Decision:** `tool-bar.vue`'s locale toggle uses `switchLocalePath(next, route.path)` from `ubean/client` + `router.push(target)` to navigate to the localized URL, NOT `useI18n().setLocale()`.
