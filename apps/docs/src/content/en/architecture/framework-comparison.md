...
- **Deliberately out of scope**: React Server Components, a multi-UI runtime, a second in-house i18n engine, **intercepting routes** (`(.)` / `(..)` / `(...)` directories — see <Link to="/guide/pages-routing/overview#dialogs-and-shareable-urls">Dialogs and shareable URLs</Link>), and Nuxt-style client `middleware/*.global` files (use `defineApp({ router: { setup } })` for guards). Need RSC? Use Next.js. Need a multi-framework content site? Use Astro.
