...
#### Application Root Component (App.vue / appRoot)

ubean's application shell is generated by the framework (`UbeanAppRoot` → layout chain → `PageView` → page). When you need another layer **above every layout** — a global ConfigProvider/theme context, an error boundary that covers layouts themselves, a global transition container, etc. — provide an "application root component": the classic Vue `App.vue` entry pattern (the counterpart of Nuxt's `app.vue`).

**Two ways to provide it (priority: `defineApp({ appRoot })` > `src/app.vue` > `src/App.vue`):**

```vue
<!-- src/app.vue — auto-detected application root component (lowercase wins;
     capitalized src/App.vue also works) -->
...
<template>
  <SConfigProvider>
    <!-- The framework outlet (layout chain + page) is injected through the
         default slot — render it with <slot /> -->
    <slot />
  </SConfigProvider>
</template>
```
...
```typescript
// app.ts — programmatic equivalent (explicit config wins over the file)
...
The component renders above the framework root component, so layouts, KeepAlive, error boundaries, and hydration all keep working. **Note:** do not render `<PageView />`/`<RouterView />` inside it — that bypasses the layout system; the outlet is always `<slot />`. The convention file accepts both lowercase `src/app.vue` (aligning with the `app.ts` entry, preferred) and capitalized `src/App.vue` (classic Vue). Only the `.vue` extension is detected — ts/js `app.ts` names belong to the defineApp entry.
...
export interface DefineAppOptions {
   ...
   * classic Vue `App.vue` / Nuxt `app.vue` entry. The framework outlet (layout
   * chain + page) is injected through the DEFAULT slot; render it with
   * `<slot />`. Priority: `defineApp({ appRoot })` > `src/app.vue` >
   * `src/App.vue`.
   */
  appRoot?: Component;
...
The DevTools client (the Vue app inside the iframe) is built with **`@vean/ui`** **+** **`@vean/aria`**:

- **`@vean/ui`**: Provides Button/Input/Select/Modal/Tree/Table/Tabs/Form/CodeEditor and other components for a unified design language
- **`@vean/aria`**: Provides unstyled functional primitives (composables, state management)
