...
`useI18n` comes from `vue-i18n` directly (auto-imported from `vue-i18n`); destructure `t` from the returned composer. Framework helpers like `setLocale` are imported from `ubean/client`.
...
In handlers, `t()` / `d()` / `n()` from `ubean` / `ubean/i18n` read the request **AsyncLocalStorage** scope. Outside a request they throw — they never fall back to a process-global locale.
