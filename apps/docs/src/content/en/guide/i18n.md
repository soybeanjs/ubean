...
`useI18n` comes from `vue-i18n` directly (auto-imported from `vue-i18n`); destructure `t` from the returned composer. Framework helpers like `setLocale` are imported from `ubean/runtime/vue` (or `ubean/client`).
