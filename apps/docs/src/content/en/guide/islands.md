...
> **Legacy syntax**: the `client:load` / `client:idle` / `client:visible` / `client:media` / `client:only` attribute syntax is still supported for backward compatibility. `v-client.*` is preferred for new code because it is a standard Vue directive — IDE and `eslint-plugin-vue` provide type-checking and autocompletion. See [Directive Reference](#directive-reference) for the full migration table.
...
1. Transforms `v-client.*` (and legacy `client:*`) directives into `<ubean-island v-once>` custom elements at build time, with data attributes carrying component info and serialized props
...
1. **Build/Dev scan**: `ubeanIslandsPlugin` scans `.vue` files for `v-client.*` and `client:*` directives
...
Both syntaxes are supported and transformed equivalently at build time. `v-client.*` is the preferred, idiomatic Vue directive form.

| Legacy (supported) | Preferred | Behavior |
| --- | --- | --- |
...
> The `v-client.media` value is a Vue expression — string literals must be quoted (`"'(max-width: 768px)'"`). The legacy `client:media="(max-width: 768px)"` treats the value as a plain string.
