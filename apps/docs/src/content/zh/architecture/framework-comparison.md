...
| Server Actions / 服务端函数 | ✅ | ❌ 一等 | ✅ form actions | ✅ | ✅ | ✅ `createServerFn` | ✅ 2.7 | ✅ `defineAction` / `defineServerFn` + `?/<name>` |
...
覆盖流式 SSR、SSG、ISR（规则层），以及 `routeRules` / `definePage` 的 `ssr`（`true` / `'data-only'` / `false` / `'streaming'`）/ `prerender` / `isr`。`ssr: false` 跳过 loader；`'data-only'` 跑 loader 但 HTML 为 CSR shell。`ppr: true` 当前等于强制流式 SSR，并参与预渲染发现；**不要把它理解成 Next.js 的 Partial Prerendering 静态壳**。纯服务端组件走 `.server.vue`（可与 `.client.vue` 配对）；客户端少发 JS 走 `v-client.*` islands。`defineServerIsland()` 把异步组件放进 `<Suspense>`，这是流式洞，不是静态壳。

### 数据与变更

- **Server Actions / Form Actions**：`defineAction()` / `defineServerFn()` + SvelteKit 风格 `?/<name>`，稳定 action ID，客户端 `callAction` / `useAction` / `useFormAction` / `invokeServerFn`。同一 `POST /__actions`，可用 `describeActionsOpenApi()` 可选暴露。
- **`useData` / `useAsyncData` / `useFetch` / `defer`**：SSR payload 水合。`useFetch` 包 `useAsyncData`；用 `setDefaultFetch(createRequest())` 注入 [`@soybeanjs/fetch`](https://www.npmjs.com/package/@soybeanjs/fetch)，框架不自研 HTTP client。
- **`after()`**：响应后回调，不阻塞 TTFB。
- Sessions 仍需显式启用。CSRF（origin）与 Data Cache 中间件默认挂载。

### 路由

文件式路由：并行路由（`@slotName/` → Vue Router named views + `<SlotView>`）、路由组、嵌套布局、`404` / `loading` / `error`。`routeRules.redirect` / `rewrite` / `proxy` / headers 均已执行。`rewrite` 内部再匹配路由；`proxy` 转发到目标 URL。
...
- CLI 脚手架：`page` / `api` / `layout` / `middleware` / `cron` 等。`ubean analyze` 读 Vite client manifest，写 `.ubean/bundle-baseline.json`。
