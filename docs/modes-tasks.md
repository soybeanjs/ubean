...
> 本文档跟踪 `mode` 配置字段(全栈/前端/后端/SSG 模式 + `ssr` 选项)的实施进度。完整方案见 [modes.md](modes.md)。
>
...
| 阶段 5:Fullstack + ssr:false | 3 | 0 | 0% |
| 阶段 6:示例与文档 | 6 | 0 | 0% |
| **合计** | **27** | **0** | **0%** |
...
> 添加 `mode` / `ssr` 类型定义、默认值和 CLI 参数解析。此阶段不改变任何构建行为,仅搭建类型基础。

- [ ] M1-1 定义 `AppMode` 类型并添加 `mode` / `ssr` 字段到 `UbeanConfig`
  - 在 [packages/config/src/types.ts](../packages/config/src/types.ts) 新增 `AppMode` 类型:`'fullstack' | 'spa' | 'ssg' | 'backend'`(不含 `ssr`,见决策 6.5)
  - 在 `UbeanConfig` 接口添加 `mode?: AppMode` 字段(含 JSDoc 注释,说明 4 种模式)
  - 在 `UbeanConfig` 接口添加 `ssr?: boolean` 字段(含 JSDoc 注释,说明仅在 `fullstack` 下生效)
  - 在 `ResolvedConfig` 的 `Required<Omit<...>>` 列表中加入 `mode` 和 `ssr`(确保 resolved 后为必填)
  - 状态:`[ ]`
  - 验证:`pnpm -F @ubean/config typecheck` 通过
  - 备注:旧的 `ssr` mode 不再支持,这是有意识的破坏性变更

- [ ] M1-2 设置 `mode` 默认值为 `fullstack`、`ssr` 默认值为 `true`
  - 在 [packages/config/src/loader.ts](../packages/config/src/loader.ts) 的 `configDefaults` 添加 `mode: 'fullstack'` 和 `ssr: true`
  - 状态:`[ ]`
  - 验证:`loadUbeanConfig()` 返回的对象 `mode === 'fullstack'` 且 `ssr === true`
  - 备注:确保现有项目行为零变化

- [ ] M1-3 CLI `build` 命令添加 `--mode` / `--ssr` / `--ssg` 参数
  - 在 [packages/cli/src/build.ts](../packages/cli/src/build.ts) 的 `args` 添加 `mode`、`ssr`、`ssg` 参数
  - `--mode` 覆盖 `ubean.config.ts` 中的 `mode`
  - `--ssr` / `--no-ssr` 覆盖 `ssr`(仅在 `--mode fullstack` 时生效)
  - `--ssg` 等同 `--mode ssg`
  - 在 `run` 函数中读取 `args.mode` / `args.ssr` 并覆盖 `config.mode` / `config.ssr`
  - 状态:`[ ]`
  - 验证:`ubean build --mode spa` 能正确读取到 `config.mode === 'spa'`
  - 验证:`ubean build --mode fullstack --no-ssr` 能正确读取到 `config.ssr === false`
  - 备注:CLI 参数优先级 > 配置文件

- [ ] M1-4 验证 `fullstack` + `ssr: true` 模式回归无变化
...
- [ ] M2-1 `buildProduction` 添加 `mode` 条件分支
  - 在 [packages/build/src/production.ts](../packages/build/src/production.ts) 的 `buildProduction` 函数中:
    ...
    - 添加 SSR bundle 条件:`(mode === 'fullstack' && ssr) || mode === 'ssg'`
    - 添加 server entry 条件:`mode === 'fullstack' || mode === 'backend'`
  - 状态:`[ ]`
  - 验证:`fullstack` + `ssr:true` 模式产物与改动前一致;`spa` 模式不产出 `dist/server/`
...
- [ ] M2-2 `spa` 模式跳过 server entry 虚拟模块
  - 在 [packages/build/src/virtual-modules.ts](../packages/build/src/virtual-modules.ts) 的 `generateVirtualModulesToDisk` 中:
    - 添加 `mode` 和 `ssrEnabled` 参数
    - `spa` 模式跳过 `server-entry.ts` / `server-entry.mjs` 的生成
    - `backend` 模式跳过页面相关虚拟模块(`vue-pages.ts`、`vue-app.ts`、`client-entry.mjs`)
  - 状态:`[ ]`
...
- [ ] M2-4 `preview` 命令支持静态文件服务
  - 在 [packages/cli/src/preview.ts](../packages/cli/src/preview.ts) 中:
    ...
    - 可使用 Node 内置 `http` + `fs`,避免引入 `sirv` 等依赖
  - 状态:`[ ]`
...
- [ ] M3-1 `ssg` 模式强制开启 prerender
  - 在 [packages/cli/src/build.ts](../packages/cli/src/build.ts) 的 `run` 函数中:
    ...
    - `config.mode === 'fullstack' && !config.ssr` 时,强制 `config.prerender.enabled = false`
  - 状态:`[ ]`
...
## 阶段 5:Fullstack + ssr:false

> 实现 `fullstack` 模式下关闭 SSR 的能力,跳过 SSR bundle 构建,保留 server bundle 用于 API 路由。

- [ ] M5-1 `fullstack` + `ssr: false` 跳过 SSR bundle 构建
  - 在 [packages/build/src/production.ts](../packages/build/src/production.ts) 中:
    - M2-1 的 SSR bundle 条件已包含 `(mode === 'fullstack' && ssr)`,此任务确认生效
    - 验证 server bundle 仍正常生成(因为 server entry 条件是 `mode === 'fullstack' || mode === 'backend'`)
  - 状态:`[ ]`
  - 验证:`fullstack` + `ssr:false` 模式 `dist/` 不含 SSR 渲染器,但含 `dist/server/`(API 路由)
  - 备注:server bundle 的 `entry.mjs` 应能正常启动 Hono app,但页面请求由客户端渲染

- [ ] M5-2 `fullstack` + `ssr: false` 时强制关闭 prerender
  - 在 [packages/cli/src/build.ts](../packages/cli/src/build.ts) 中:
    - `config.mode === 'fullstack' && !config.ssr` 时,强制 `config.prerender.enabled = false`
  - 状态:`[ ]`
  - 验证:`fullstack` + `ssr:false` 模式构建日志不包含 "Prerendering"
  - 备注:无 SSR bundle 无法 prerender,逻辑上必须关闭

- [ ] M5-3 验证 `fullstack` + `ssr: false` 的 server bundle 行为
  - 启动 `dist/server/server.mjs`,验证:
    - API 路由(`/api/**`)正常响应
    - 静态文件(`/assets/**`、`/favicon.svg` 等)正常服务
    - 页面请求(`/`、`/about` 等)返回 `index.html`(由客户端渲染)
  - 状态:`[ ]`
  - 验证:`curl http://localhost:3000/api/users` 返回 JSON;`curl http://localhost:3000/` 返回 HTML
  - 备注:可能需要修改 server entry 的页面处理逻辑,当 `ssr: false` 时直接返回 `index.html`
...
- [ ] M6-3 更新 AGENTS.md 添加 mode 说明
  - 在 [AGENTS.md](../AGENTS.md) 的 "3. 核心约定" 章节新增 "3.7 应用模式" 小节:
    - 列出 4 种 mode 及其用途
    - 说明 `ssr` 选项(仅 `fullstack` 模式下生效)
  ...
  - 在 "8. 常见陷阱" 新增:
    ...
    - "不要使用 `mode: 'ssr'` — 已移除,改用 `mode: 'fullstack'` + `ssr: false`"
  - 状态:`[ ]`
  - 验证:AGENTS.md 内容准确、与实现一致
  - 备注:AGENTS.md 是 AI 助手的主要参考,需保持更新

- [ ] M6-4 更新 skills/ubean 文档
  - 在 [skills/ubean/docs/guide/](../skills/ubean/docs/guide/) 新增 `modes.md`:
    - 面向用户的模式选择指南(何时用 spa/ssg/backend/fullstack+ssr:false)
    - 各模式的配置示例
    - 各模式的产物结构说明
  - 更新 [skills/ubean/SKILL.md](../skills/ubean/SKILL.md) 提及 `mode` 和 `ssr` 字段
  - 状态:`[ ]`
...
- [ ] M6-6 全量回归验证
  ...
  - 验证 `examples/ubean-test`(fullstack + ssr:true 模式)产物结构与改动前一致
  ...
  - 验证 `fullstack` + `ssr:false` 模式跳过 SSR bundle 且 API 路由可响应
  - 状态:`[ ]`
                      ...
                      M2-1 → M2-2 → M2-3 → M2-4 → M2-5(spa 示例)
                                           ...
                                           M3-1 → M3-2 → M3-3 → M3-4 → M3-5(ssg 示例)
                                                                  ...
                                                                  M4-1 → M4-2 → M4-3 → M4-4(backend)
                                                                                           ↓
                                                                                      M5-1 → M5-2 → M5-3(fullstack + ssr:false)
                                                                                                                ↓
                                                                                                           M6-1 ~ M6-6(文档)
```
...
- Phase 5(fullstack + ssr:false)依赖 Phase 2 的 `buildProduction` 重构
- Phase 6 必须在所有功能阶段完成后

## 风险与缓解

| 风险 | 影响 | 缓解措施 |
|---|---|---|
| `buildProduction` 重构引入回归 | 高 | M1-4 建立回归基线;每次提取子函数后立即验证 `fullstack` + `ssr:true` 产物 |
...
| `fullstack` + `ssr:false` server entry 行为异常 | 中 | M5-3 验证 API 路由 + 静态文件 + 页面 fallback 三种场景 |
| 静态文件服务器引入新依赖 | 低 | 优先用 Node 内置 `http` + `fs`,避免引入 `sirv` 等 |
| CI 构建矩阵膨胀 | 低 | examples 数量有限(4-5 个),可控 |

## 附录:产物结构对照

### fullstack + ssr:true(默认)
