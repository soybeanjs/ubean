...
- **Phase 3 收尾**：`RM-V26`（cloudflare preview）✅ runner / 🟡 产物受阻 —— `createCloudflarePreviewRunner()` 把产物 worker 交给 miniflare 在进程内跑（可选 peer，缺失时给出安装提示），预览中间件按 preset 分流。miniflare 真机实测通过（合成 worker：派发 / POST 体 / dispose），但依赖**不进仓库**（workerd 体积大，同既有可选依赖约定），故 CI 里真机用例跳过、接线层由注入假 loader 的用例守着（7 条）。**连带撞出三个产物级问题**：① 无用户 `vite.config` + `backend` 模式构建失败（`virtual:ubean-app` 只在 `hasPages` 时注册，而服务端入口无条件 import 它；矩阵的 backend 格有配置、无配置格是 fullstack —— **交叉处从未被测过**），已修；② **cloudflare 产物在 workerd 里起不来**（模块图带 `node:fs/promises`，workerd 不支持 → 该 preset 的产物在 Cloudflare 上同样跑不起来，此前从未真正运行过；当轮把失败前移为构建期审计告警，**随后已修** —— 见「Phase 6 / 缺陷收尾」）；③ 配置里的 `preset` 顶层字段从不被读取（实际是 `build.preset`，文档与实现不一致，登记到 RM-V32）。
- **Phase 4**：`RM-V27`（编译期资产清单）✅ 评估结论：**不引入** —— RM-V18 已达成同一目标（标签构建期内联成字面量、SSR 路径无运行时读盘），`?assets` 只会替换一个不存在的问题。`RM-V28`（跨环境单例代理）/ `RM-V29`（services 环境机制）🟡 评估结论：**暂不采用** —— 二者服务于「多 dev 环境」拓扑，而**当前 dev 只跑一个环境**：`UbeanDevEnvironment`（RM-V08/V09 的能力与测试都在）**没有接进实际路径**，全仓无 `createEnvironment` 注册点，RM-V14 后的 `dev-vite.ts` 在主进程跑 SSR 图。单例要求在现有拓扑下由 `ssrSingletonDevPolicy()` 满足，并有 reload 作用域探针（5/5 保留）作证据；已把 `dev-environment.ts` 文件头「dev 下服务端代码在 worker 里执行」的说法改成与实现一致。触发条件写明：谁先需要第二个 server 环境，谁把它们一起接线。`RM-V30`（平台 runner）✅ cloudflare（RM-V26）/ 🟡 vercel·netlify 不做（产物是标准 fetch handler，node 路径已能完整预览，再包一层换不到保真度）。`RM-V31`（裸 `vite` 命令）✅ —— 三条命令都有了端到端用例，本轮补上唯一缺的 `vite build`（`vite-build.test.ts`：产物完整度而非退出码 —— 服务端 bundle / node 包装 / manifest / 预渲染 HTML / 岛屿产物 / 内联入口 script）。
- **Phase 5 收口**：`RM-V32…V36` ✅ —— ① AGENTS.md（含查实的文档错误：预设写在 `build: { preset }`，**没有**顶层 `preset` 字段）；② 站点文档中英（`dist/client/.vite/manifest.json` 笔误 + 过期体积快照 113.0/6.6/30 → 111.1/45.2/32 + 两条易踩坑）；③ skill 的「Equivalent Vite commands」表；④ 新增 `docs/migration-guide-vite-plugin.md`（用户视角：要改什么、已生效的行为差异、收敛与回滚）；⑤ **双轨收敛**：开关默认打开 → 旧编排删除（`buildProduction` 188 行、`experimental` 配置字段与全部读取点、`build-parity.test.ts`），矩阵重写为按 mode × preset 断产物契约（15 格）。收敛过程本身又暴露两个缺陷：`buildWithEnvironments()` 在开关打开后**静默返回 undefined**（插件 `buildApp` 覆盖内联编排），以及 **spa 产物从来没有 `index.html`**（与文档承诺不符，部署出去没有入口文件）。
- **Phase 6 / 缺陷收尾**：**cloudflare 产物在 workerd 里起不来的问题已修**（原先登记为「需独立一笔」）—— 十条原因与修法列在 [vite-plugin-migration.md](vite-plugin-migration.md) 的缺陷 D 表里，可归为四类：服务端 barrel 带的 Node 专用工具（`node:net` / `node:os`）、Node-only 能力未条件化（静态服务 / fs 缓存 / SEO 扫描）、worker 构建配置（全量打包、CJS 垫片的 `import.meta.url`、`process.env` / `global` 垫片、`nodejs_compat` 与兼容性日期）、以及 `--preset` 对插件实例不可见。验收：`cloudflare-preview.test.ts` 新增**真机用例**（构建 → miniflare 启动 → `/` SSR HTML / `/hello` JSON / `/_health`），另有 7 条配置面单测锁住判据；完整示例（20 页 / 63 API）手工验证 SSR 200 / API 200 / 预渲染 200 / 404 页 404。
- **Phase 6 · preset 元数据向实现对齐**：11 个内置 preset 声明的 `output.dir` / `output.serverDir` / `output.publicDir` / `build.outputDir` 与 `runtime.entry` 全部改成真实产物（`dist` / `dist/server` / `dist/public` / 该 entryType 对应的包装文件），并加守卫测试断言「声明即真实产物」。**只改元数据、不碰产物布局** —— 让 `getBuildOutDirs` 去尊重 `output.serverDir` 会同时改变 11 个 preset 的布局，属破坏性变更。
- **Phase 6 · 文档承诺 vs 实际覆盖审计收口**：审计清单上的最后三项（PPR / 服务端岛屿 / 流式延迟数据）已从「文档写了」变成可执行断言，dev 与生产两侧各有用例。过程里又撞出两个**只在生产暴露**的缺陷：**M**（宏剥离把 `<script setup>` 剥空 → 「loader 写 `<script>`、`definePage` 写 `<script setup>`」这一官方约定形状在生产构建报 `MISSING_EXPORT "default"`，dev 侥幸通过）与 **N**（`useDeferredData` 在 setup 期同步应用 payload → 客户端首帧与 SSR 分支不一致，Vue 报 hydration mismatch 且元素留着 SSR 的旧 class）。同时把 `defer` / `useDeferredData` 从 `ubean/client` 导出（此前只有聚合入口一条路，与客户端导入纪律冲突）。示例体积基线在本批重建 —— 它自审计批次新增示例页后就没再刷新，HEAD 上 `analyze:check` 已是红的；本批用隔离构建量出自身增量 +2.1 kB gzip（+1.9%）后刷新，归因与操作陷阱见 [perf-regression-net.md](perf-regression-net.md)。审计的最后一项（拦截路由只有元数据、没有运行时）已按 [ADR-0010](adr/0010-competitive-north-star-and-gap-filter.md) 判为**刻意不做**并清理干净（约定痕迹全删、扫到标记段抛错、站点对比矩阵改正、给出并行路由的替代做法）。
- **Phase 6 · 浏览器走查补齐**：`RM-V15` 那条「已知缺口」已关闭 —— 示例补了页面级 Server Action 表单 （`src/pages/action-demo.vue`），`dev-dx.test.ts` 新增浏览器用例（点击 → 无整页刷新 → 结果上屏 → 字段错误回填），另有原生表单提交的无 JS 路径（服务端渲染结果 / 422 字段错误）经 HTTP 验证。**补的过程中查出并修掉一个真缺陷**：SFC 页面里的 `export const actions` 在客户端从未被替换（插件按后缀过滤、又只处理 `?type=script` 子请求，而 Vue 会把脚本块 import 提升进主模块），表现为该页在浏览器里永不水合 —— 详见 [vite-plugin-migration.md](vite-plugin-migration.md) 的缺陷 G。另给 `ubean preview` 补了 `--outDir`（未知参数会被 citty 静默忽略，排查时正因此被误导）。
