...
| P6-03 | Route Rules                | ✅   | P1     | `src/runtime/route-rules.ts`                                      | ✅ 路径模式编译（`*`单段/`**`多段通配符）、headers/redirects/rewrites、多规则合并（specificity排序）、cache规则转Cache-Control、createRouteRulesMiddleware中间件集成到app；proxy/cors待后续实现 |
