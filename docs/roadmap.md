...
| P2-04b | Queues 扫描                         | ✅   | P1     | `src/core/routing/scan.ts` (queues 扫描集成)                    | queues/**/*.{ts,js,mjs,cjs,mts,cts} 扫描，提取队列名称、支持序号前缀                                                                           |
| P2-04c | Locales 扫描                        | ✅   | P1     | `src/core/routing/scan.ts` (locales 扫描集成)                    | locales/**/*.{json,yaml,ts,js} 扫描，提取语言代码与默认标记                                                        |
...
| P6-10 | DevTools 基础架构          | ✅   | P1     | `src/core/devtools/`                                       | ✅ DevTools类型定义(DevToolsInfo/DevToolsRouteInfo/DevToolsPageInfo/DevToolsMiddlewareInfo/RpcRequest/RpcResponse)、createRpcServer RPC服务器(registerHandler/handleRequest/handleBatch/updateInfo/setRoutes/setPages/setMiddlewares、内置ping/getInfo/getRoutes/getPages/getMiddlewares方法)、createDevToolsMiddleware Hono中间件(RPC端点/iframe端点/HTML响应脚本注入/__ubean_devtools__防重复注入标记)、客户端浮动按钮+侧滑面板+iframe UI(基础实现)、createUbeanApp集成devtools选项、dev命令自动启用DevTools并传入扫描到的routes/pages/middlewares信息、28个测试(共610全部通过)；Vite插件增强、birpc双向通信、内置Tab(使用@soybeanjs/ui重构)、CRUD操作、自定义Tab等功能待后续P6-11~P6-16实现；UI规范见docs/engineering.md §9.1 |
