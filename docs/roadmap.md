...
- **Phase 1**：`RM-V15`（dev 全量验收）🟡 自动化部分完成 —— CLI 134 + example 783 + 4 条真 dev server 用例 + 浏览器运行时（水合 116ms vs 基线 155、导航 73ms vs 121）全绿，开关两侧对照与 `RM-P02` 生效证明（`viteBuilder` 臂改为跑 `vp dev`）均落地；剩余人工 DX 走查清单。
