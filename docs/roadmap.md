...
- **Phase 6 · 文档承诺 vs 实际覆盖审计收口**：审计清单上的最后三项（PPR / 服务端岛屿 / 流式延迟数据）已从「文档写了」变成可执行断言，dev 与生产两侧各有用例。过程里又撞出两个**只在生产暴露**的缺陷：**M**（宏剥离把 `<script setup>` 剥空 → 「loader 写 `<script>`、`definePage` 写 `<script setup>`」这一官方约定形状在生产构建报 `MISSING_EXPORT "default"`，dev 侥幸通过）与 **N**（`useDeferredData` 在 setup 期同步应用 payload → 客户端首帧与 SSR 分支不一致，Vue 报 hydration mismatch 且元素留着 SSR 的旧 class）。同时把 `defer` / `useDeferredData` 从 `ubean/client` 导出（此前只有聚合入口一条路，与客户端导入纪律冲突）。示例体积基线在本批重建 —— 它自审计批次新增示例页后就没再刷新，HEAD 上 `analyze:check` 已是红的；本批用隔离构建量出自身增量 +2.1 kB gzip（+1.9%）后刷新，归因与操作陷阱见 [perf-regression-net.md](perf-regression-net.md)。仍待 owner 定的只剩拦截路由的运行时语义（生成器产出 `meta.interceptFrom` / `interceptTarget` / `isIntercepting`，全仓无消费者）。
