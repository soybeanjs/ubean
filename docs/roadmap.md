...
- **Phase 1 起步**：`RM-V11`（宿主 dev app）🟡 —— 先抽出 `buildDevSsrRoutes()`（SSR 路由表，含 404 catch-all / reuse / locale param），CLI 已改用它；测试直接把服务端路由表与客户端 `generatePagesModulePages` 产出对照，让「两张表同形」成为可执行约束（R8 与 locale 前缀事故都源于此）。剩余部分（app 自举 + `enhanceAppWithVite` 搬入 builder）待续。
