...
- [ ] T0-1 创建 `packages/` 下 23 个新子包目录骨架
...
- [ ] T0-2 配置 `tsconfig.json` 的 `references`,支持子包增量编译
...
- [ ] T0-3 验证 `pnpm-workspace.yaml` 已包含新子包
...
- [ ] T1-1 迁移 `packages/ubean/src/types/handler.ts` → `packages/types/src/`
...
- [ ] T1-2 迁移 `packages/ubean/src/utils/path.ts` → `packages/utils/src/`
  - 包含:`filePathToRoute`、`stripRouteGroups`、路由路径转换工具
  - 状态:`[ ]`
  - 验证:`pnpm -F @ubean/utils typecheck` 通过
  - 备注:

- [ ] T1-3 迁移 `packages/ubean/src/core/utils/{port,vite-config}.ts` → `packages/utils/src/`
...
- [ ] T1-4 迁移 `packages/ubean/src/core/markdown/` → `packages/markdown/src/`
...
- [ ] T1-5 迁移 `packages/ubean/src/runtime/error.ts` → `packages/error/src/`
...
- [ ] T1-6 迁移 `packages/ubean/src/runtime/env.ts` → `packages/env/src/`
...
- [ ] T1-7 迁移 `packages/ubean/src/runtime/seo.ts` → `packages/seo/src/`
...
- [ ] T1-8 迁移 `packages/ubean/src/runtime/{i18n,i18n-routing}.ts` → `packages/i18n/src/`
...
- [ ] T1-9 迁移 `packages/ubean/src/runtime/client.ts` → `packages/client/src/`
...
- [ ] T1-10 迁移 `packages/ubean/src/runtime/pages/` → `packages/pages/src/`
...
- [ ] T2-1 迁移 `packages/ubean/src/core/routing/{scan,router,route-name,detect-exports,types}.ts` → `packages/routing/src/`
...
- [ ] T2-2 迁移 `packages/ubean/src/core/routing/define-page.ts` AST 部分 → `packages/routing/src/define-page.ts`
...
- [ ] T2-3 实现 `@ubean/routing/generator` 实体文件生成器 ⭐
...
- [ ] T2-4 迁移后端 API 路由运行时 → `packages/api-routes/src/`
...
- [ ] T2-5 合并迁移服务端运行时原语 → `packages/server-runtime/src/`
...
- [ ] T2-6 迁移 Hono 应用工厂 → `packages/app/src/`
...
- [ ] T2-7 增强 `@ubean/config` 新增 `RoutingConfig` 类型
...
- [ ] T3-1 迁移 `packages/ubean/src/runtime/vue/` → `packages/runtime/src/`
...
- [ ] T3-2 迁移 Islands 架构 → `packages/islands/src/`
...
- [ ] T3-3 迁移 Vue SSR 渲染器 → `packages/ssr/src/`
...
- [ ] T3-4 迁移 Vue 专属 Vite 插件 → `packages/vite/src/`
...
- [ ] T4-1 迁移自动导入预设 → `packages/auto-imports/src/`
...
- [ ] T4-2 迁移 SSG 预渲染 → `packages/prerender/src/`
...
- [ ] T4-3 迁移部署预设 → `packages/preset/src/`
  - 来源:`core/preset/`
...
- [ ] T4-4 迁移类型生成 → `packages/codegen/src/`
  - 来源:`core/codegen/`
...
- [ ] T4-5 迁移模块系统 → `packages/modules/src/`
  - 来源:`core/modules/`
...
- [ ] T4-6 迁移核心 Vite 插件(框架无关部分)→ `packages/build/src/`
...
- [ ] T4-7 迁移开发服务器编排 → `packages/dev/src/`
  - 来源:`core/dev/`
...
- [ ] T4-8 迁移 CLI 命令 → `packages/cli/src/`
...
- [ ] T5-2 实现 `@ubean/core/vite` 默认 Vite 插件组装
...
- [ ] T6-1 新增 `examples/frontend-only/` 示例
  - 场景 B 示例(SPA,无后端)
...
- [ ] T6-2 新增 `examples/routing-file-mode/` 示例
  - 路由实体文件模式示例
...
- [ ] T6-3 升级 `examples/ubean-test/` 为 `routing-virtual-mode/` 示例
...
- [ ] T6-5 更新 `skills/ubean/docs/` 增加路由生成模式文档
...
- [ ] T7-1 删除 `packages/ubean/src/` 下原代码
  - 所有源码已迁移到子包
...
- [ ] T7-2 `packages/ubean/src/index.ts` 改为纯 re-export
  - 内容:`export * from '@ubean/core';` + `export type * from '@ubean/core';`
  - 状态:`[ ]`
  - 验证:`pnpm -F ubean typecheck` 通过
  - 备注:

- [ ] T7-3 `packages/ubean/bin/ubean.mjs` 改为转发
...
- [ ] T7-4 `packages/ubean/package.json` 改造
...
- [ ] T7-5 `packages/core/bin/ubean-next.mjs` 改名为 `ubean.mjs`
...
- [ ] T7-6 端到端验证
  - `examples/ubean-test` 跑通 `pnpm dev` + `pnpm build`(使用 `ubean` CLI)
  - `examples/frontend-only` 跑通
  - `examples/routing-file-mode` 跑通
  - API 表面 100% 向后兼容
  - CI 全绿
...
- [ ] T8-1 在 `packages/ubean` 中将 `@ubean/core` 标记为推荐入口
  - 在 `packages/ubean/README.md` 中标注推荐使用 `@ubean/core`
  - 状态:`[ ]`
  - 备注:可选

- [ ] T8-2 评估是否合并 `packages/ubean` 与 `packages/core`
