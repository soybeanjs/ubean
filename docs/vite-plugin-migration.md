...
| **RM-V15** 🟡 进行中 | dev 全量验收 | 自动化部分已覆盖：CLI 134（拓扑 22 含 SSR HTML / 页面 404 HTML / API 404 JSON / 安全头 / CSRF / i18n cookie / devtools 302）、example 783（i18n、islands、Server Actions、cron、DevTools、rate-limit、数据缓存等按功能域）、`dev-reload.test.ts` 4 条真 dev server 用例（旧路径热重载、开关过渡路径、`vp dev` 四类请求、`vp dev` 热重载）、浏览器运行时经基准脚本的 Chromium 阶段（水合 / 站内导航 + 服务端单例保留） | **实测（开关打开的新路径）**：冷启动 1.63s、服务端变更 p50 222ms 与基线一致、客户端变更 105ms、水合 **116ms**（基线 155ms）、站内导航 **73ms**（基线 121ms）、reload 单例保留 3/3 与 5/5（两次采集）——四项观测率均达标，浏览器指标优于基线。**判定**：Phase 1 具备按 `experimental.viteBuilder` 灰度发布的条件。**剩余**：人工 DX 走查清单（浏览器内切语言、表单提交 Server Action、DevTools 面板交互、HMR 状态保持的主观确认）尚未逐条执行，自动化测试覆盖了对应行为但不等价于人工确认 |
