...
| **RM-V14** 🟡 部分 | `ubean dev` 薄别名 | **已达成行为等价**：`ubeanVite` 接管 `@vitejs/plugin-vue` 的注册（可 `vue: false` 关闭），CLI 不再重复注册 —— 此前 `.vue` 的编译由 CLI 代劳，于是同一份 `ubeanPlugin()` 在 `ubean dev` 下可用、在 `vite dev` 下直接报 “Install @vitejs/plugin-vue”；`experimental.viteBuilder` 打开时聚合入口追加 `ubeanDevRequestPlugin()`，`vite dev` 因此独立可服务应用；插件自举的 app 订阅扫描协调器，改动/新增路由文件都会重建（`bootstrapDevApp().rebuild()`）。示例项目用 `UBEAN_VITE_BUILDER=1` 打开该开关供集成测试走这条路径 | 测试：`packages/cli/test/dev-reload.test.ts` 增加「`vp dev`（开关打开）服务页面 SSR / API / 内置 `_` / 404 四类请求」+「改文件同样生效」两个用例（子进程 + 纯 HTTP）。**实测**：服务端变更 p50 **222ms**（9 次采样，与基线一致）、客户端变更 105ms、冷启动 1.65s、reload 单例保留。**注意 p95**：该指标呈双峰（~220ms / ~325ms），p95 327ms 高于基线的 223ms，慢模式约 +105ms 且在 RM-V10 的采集里也出现过（422ms 离群），故非本次引入，交由 RM-V15 定位。**剩余**：把 `dev.ts` 真正改成调 Vite `createServer` 并删除 `dev-server/vite-server.ts` 与 `runner.ts`（约 670 行）—— 功能上两者已等价，这步是机械性收敛 |
