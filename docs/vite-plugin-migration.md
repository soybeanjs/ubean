...
**同批查实的另一条：拦截路由只有元数据、没有运行时 —— 已按 ADR-0010 判为「刻意不做」并清理（2026-09-17）。** 当时生成器会为 `(..)target/` 之类的文件注册带 `meta.interceptFrom` / `interceptTarget` / `isIntercepting` 的路由，但全仓没有任何**消费者** —— 「从 X 导航到 Y 时渲染拦截页」这件事不会发生，拦截页只能靠它自己被清理后的路径访问（而且那条路径是 `/feed/:id` 这种谁也不会访问的废 URL；`interceptTarget` 当时也只是标记后的片段文本，不是目标 URL）。逐项过 ADR-0010 的门槛后结论是**刻意不做**：不是架构还债、不涉性能，「用户习惯缺口」也不成立（`(.)` 是 Next 专属约定，Nuxt 没有对应物），唯一论据只剩「Next 有」；而接线要求**故意的 SSR/客户端分叉**（与本仓在 J 缺陷里刚立起的「两侧一致」原则冲突），背景页还会随拦截记录重挂载。核心价值（URL 可分享、back 关闭对话框）用已有的并行路由 + 一个守卫在应用侧就能拿到。

清理动作：扫描器的标记段解析与两个元数据字段、生成器的 `__intercept_*` 注册、dev SSR 与产物入口里的两处同名分支、`serializePagesForEntry` 白名单里的两个字段、相关测试全部移除；**扫到标记段改为抛错**（不拦的话它会退化成 `/feed/(.)photo/:id` 字面垃圾路径 —— 路由组正则只吞整段 `(group)/`），错误信息给出替代做法；站点中英 `framework-comparison.md` 从「已接线」里删掉该项。机制上若将来要重做，可行性已实测：同一 URL 两条记录、按路径解析落真实页（直链/刷新渲染完整页）、按名字解析落拦截记录（软导航渲染对话框），守卫须**先判断 `to.name` 前缀**防重定向环，且背景页必须是构建期静态可解的（在守卫里动态 `addRoute` 实测进重定向环并丢参数）。
