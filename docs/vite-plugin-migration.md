...
| **RM-V11** 🟡 起步 | 宿主 `UbeanDevApp` | 迁移 `vite-server.ts` 的 bootstrap（`ssrLoadModule('virtual:ubean-server')` → `resolveServerConfig('dev')` → `applyServerConfig`）及 devtools、错误页、`/_openapi.json`、VFS 到宿主 dev app。**已抽出部件**：`packages/builder/src/dev/dev-ssr-routes.ts` 的 `buildDevSsrRoutes()` —— 服务端 SSR 路由表（含 404 catch-all、reuse、locale param），CLI 已改为调用它 | 已落地部件：`test/dev-ssr-routes.test.ts` 10 断言，其中**直接解析客户端 `generatePagesModuleSource` 的真实产出做对照**，把「两张表同形」变成可执行约束（R8 与 locale 前缀两次事故的根因都是两表不一致，靠注释互相提醒是不够的）。client/cli/example 全绿。**剩余**：把 app 创建与 `enhanceAppWithVite`（routeLoaders/pageLoaders/middlewareLoaders/pageRenderer/crons/islands）搬进 builder，让插件在无 CLI 时自举（需给 builder 加 `@ubean/app` 依赖，注意 R9 的 peer 变体漂移） |
