...
| **RM-V09** 🟡 部分 | dev worker entry | 新增 `packages/builder/src/dev/dev-worker.ts`：**框架生成** worker 入口到项目 `.ubean/dev-worker.mjs`（避免 `.mjs` 资源被打包器漏掉、且与框架版本天然一致），模板支持注入已解析的 `env-runner/vite` / `vite/module-runner` 定位符（否则生成物在项目目录里解析不到包）；worker 内 `ModuleRunner` + `transformHTML` 之外的 `ubean:invalidate` 协议与宿主侧 `invalidateDevWorkerModules()`（宿主模块图 + worker 两侧失效） | worker 加载宿主模块并处理请求 ✅（`packages/cli/test/dev-worker.test.ts`）；**失效路径未跑通**，两个根因已实测确认：① 模块图按**规范化真实路径**索引（macOS 上 `tmpdir()` 是符号链接，`/var/...` 的 realpath 是 `/private/var/...`），用原路径 `getModulesByFile()` 匹配不到任何条目；② 生成器模板中的**嵌套模板字符串必须转义**（`\${}`），否则被外层模板插值导致 worker 侧 ReferenceError。用例显式 skip 并写明诊断与下一步（宿主失效改为「拿到模块 URL 再发给 worker」，URL 是两侧唯一稳定的键） |
