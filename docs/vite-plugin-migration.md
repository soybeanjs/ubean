...
| **RM-V11** 🟡 部分 | 宿主 `UbeanDevApp` | 迁移 app 侧 bootstrap（`ssrLoadModule('virtual:ubean-server')` → `resolveServerConfig('dev')` → `applyServerConfig`）及 devtools、错误页、`/_openapi.json`、VFS 到宿主 dev app。**已搬入 builder 的部分**：① `packages/builder/src/dev/dev-ssr-routes.ts` 的 `buildDevSsrRoutes()`（SSR 路由表：catch-all / reuse / locale param / 布局解析）；② `packages/builder/src/dev/dev-host-app.ts` 的 `enhanceDevApp()`（layouts + routeLoaders/pageLoaders/middlewareLoaders + cron 预加载 + `pageRenderer` 接线），CLI 侧 `enhanceAppWithVite` 已退化为一次调用 | 已落地：`test/dev-ssr-routes.test.ts`（10 断言，**直接解析客户端 `generatePagesModuleSource` 产出做对照** —— 服务端与客户端两张路由表同形成为可执行约束，R8 与 locale 前缀两次事故的根因都是两表不一致）；`test/dev-host-app.test.ts`（6 断言：加载器键与目标文件、reuse 指向目标页、渲染器收到含 catch-all 的路由表、backend 分支 `resetInit`、cron 加载后再启动调度器、cron 失败走 `onCronError`）。依赖面用结构化类型（`DevHostAppLike`）而非 `import type { UbeanApp }`，builder 不必为此新增依赖（对齐 `EnvRunnerLike` 的既有做法）。**剩余**：app 本身的创建（`createUbeanApp` + scan + codegen）、devtools 插件、错误页、VFS 仍需搬入/自举，之后插件才可脱离 CLI 独立服务 `vite dev` |
