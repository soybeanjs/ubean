...
import { useI18n, useSwitchLocalePath, useLocalePath, setLocale, getI18nRuntimeConfig } from 'ubean/runtime/vue';
...
const { locale, t, fallbackLocale, d, n } = useI18n();
const localeCodes = computed(() => getI18nRuntimeConfig()?.locales ?? ['en', 'zh']);
  ...
  <div class="p-8 max-w-4xl mx-auto">
    ...
    <section class="mb-8 p-4 border rounded-lg">
      ...
      <div class="grid grid-cols-2 gap-2 text-sm">
        ...
        <div class="font-mono">{{ localeCodes.join(', ') }}</div>
      </div>
    </section>
    ...
    <section class="mb-8 p-4 border rounded-lg">
      ...
      <div class="space-y-1 text-sm">
        <div>count=0: {{ t('items.count', 0) }}</div>
        <div>count=1: {{ t('items.count', 1) }}</div>
        <div>count=5: {{ t('items.count', 5) }}</div>
      </div>
    </section>
    ...
    <section class="mb-8 p-4 border rounded-lg">
      ...
      <div class="space-y-3 text-sm">
        ...
        <div>
          ...
          <div>{{ new Intl.ListFormat(String(locale)).format(testItems) }}</div>
        </div>
      </div>
    </section>
    ...
    <section class="mb-8 p-4 border rounded-lg">
      ...
      <div class="flex gap-3 text-sm">
        <Link to="/" class="text-blue-500 hover:underline">{{ t('navigation.home') }}</Link>
        <Link to="/about" class="text-blue-500 hover:underline">{{ t('navigation.about') }}</Link>
        <Link to="/features" class="text-blue-500 hover:underline">{{ t('navigation.features') }}</Link>
        <Link to="/dashboard" class="text-blue-500 hover:underline">{{ t('navigation.dashboard') }}</Link>
      </div>
    </section>
  </div>
</template>
