...
export function createVueAppEntryVirtualModule(appEntry: ScannedAppEntry = EMPTY_APP_ENTRY) {
  return defineVirtualModule('virtual:ubean-app', () => {
    ...
    // src/app.vue(小写,优先)/ src/App.vue — 应用根组件(包装组件)
    // 自动检测回退。静态 import(与 app.ts 配置入口一致):根组件在语义上
    // 同步存在,无需像 loading/error 特殊页那样懒加载。
    const rootAppImport = hasRootApp
      ? `import _rootApp from ${JSON.stringify(appEntry.root!.fullPath)};`
      : 'const _rootApp = null;';
