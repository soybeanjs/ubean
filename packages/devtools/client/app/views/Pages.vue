...
<template>
  <div class="flex flex-col h-full">
    <div class="px-3.5 py-2 border-b border-base bg-background flex items-center gap-2 shrink-0">
      ...
      <span class="text-2xs text-muted-foreground ml-auto shrink-0 op-fade">{{ filteredPages.length }} pages</span>
      <button
        class="inline-flex items-center gap-1 px-2 py-1.5 rounded-md bg-primary/10 text-primary text-xs font-medium hover:bg-primary/20 transition-colors cursor-pointer shrink-0"
        title="New page (P)"
        @click="emit('create')"
      >
    ...
    <div class="flex-1 overflow-y-auto p-3.5">
      <div v-if="filteredPages.length > 0" class="flex flex-col gap-1">
        <div v-for="(p, i) in filteredPages" :key="i" class="list-item group">
          <SIcon icon="lucide:file-text" :size="14" class="text-primary shrink-0" />
          ...
          <button
            class="size-6 flex items-center justify-center rounded-md text-muted-foreground hover:text-destructive hover:bg-destructive/10 opacity-0 group-hover:opacity-100 transition-all cursor-pointer shrink-0 ml-1"
            title="Delete page"
