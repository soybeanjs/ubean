...
export async function prerender(options: PrerendererOptions): Promise<PrerenderResult> {
  ...
  logger.info(`Prerendering ${queue.length} initial routes...`);
    ...
    try {
      if (options.fetcher) {
        ...
        if (resp.statusCode >= 400) {
          ...
          logger.warn(`  ⚠ ${route} → ${resp.statusCode}`);
          return;
        }
        ...
        logger.debug(`  ✓ ${route} → ${relative(options.cwd, filePath)}`);
      } else {
...
<div id="app"><!-- Prerendered content placeholder for ${route} --></div>
        ...
        logger.debug(`  ✓ ${route} (placeholder)`);
      }
    } catch (err) {
      ...
      logger.warn(`  ⚠ ${route} → ${result.error.message}`);
    }
