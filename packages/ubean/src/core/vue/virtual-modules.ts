...
export function createVuePagesVirtualModule(pages: ScannedPageRoute[], layouts: ScannedLayout[]) {
  return defineVirtualModule('virtual:ubean-pages', () => {
    ...
    for (const p of pages) {
      ...
      routeEntries.push(
        `  { path: ${JSON.stringify(routerPath)}, name: ${JSON.stringify(p.name)}, component: ${varName}, meta: { layout: ${layoutValue}, pageName: ${JSON.stringify(p.name)} } }`
      );
    }
...
const _layoutLoaders: Record<string, () => Promise<any>> = {
${layoutLoaderMap}
};

const _pageLoaders: Record<string, () => Promise<any>> = {
...
import type { ResolvedAppConfig } from 'ubean/runtime/vue';
...
export type { RouteName, LayoutName } from 'virtual:ubean-pages';

function _mergeAppConfig(base: ResolvedAppConfig, ...configs: (ResolvedAppConfig | null | undefined)[]): ResolvedAppConfig {
...
export function resolveAppConfig(mode: 'client' | 'server'): ResolvedAppConfig {
...
export function createApp(): { app: App; router: Router } {
...
import type { ResolvedServerConfig } from 'ubean/runtime/app';
 ...
 */
export function resolveServerConfig(mode: 'dev' | 'prod'): ResolvedServerConfig {
