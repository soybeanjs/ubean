...
export async function generateRouteFiles(scan: GeneratorScanInput, options: GeneratorOptions): Promise<GeneratorResult> {
...
`;
  ...
  renderDtsFile(scan: GeneratorScanInput): string {
    ...
    const pathMapEntries =
      scan.pages.length > 0 ? scan.pages.map(p => `    "${p.name}": "${p.route}";`).join('\n') : '    // (no pages scanned)';
