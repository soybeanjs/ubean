...
const ARMS = {
  cli: {
    label: 'cli',
    describe: '`ubean dev` / `ubean build`（CLI 入口）',
    /**
     * ADR-0012 收敛后两个入口**共用同一套生命周期**（请求路由与构建编排都在插件里），因此本臂
     * 与 `vite` 臂测的不再是两种实现，而是**入口本身的开销**（`pnpm exec ubean` 的 CLI 启动 vs
     * `pnpm exec vp` 的 vite 启动）。保留两臂是为了让这个开销可见 —— 实测 `ubean --version`
     * 0.73–1.23s 而 `vp --version` 0.09s（CLI 的急切 import 图不便宜）。
     */
    env: () => ({}),
  ...
  vite: {
    label: 'vite',
    describe: '`vp dev` / `vp build`（裸 Vite 入口，无 CLI 参与）',
    env: () => ({}),
    devCommand: () => ['exec', 'vp', 'dev'],
    buildCommand: () => ['exec', 'vp', 'build'],
    /**
     * 生效证明（RM-P02）：没有 CLI，请求路由与宿主 app 只可能来自插件 —— 这一臂的数字必然出自
     * 插件路径。实测 `vp dev` 未生效时会 404，因此这里用 SSR 页面做硬断言，避免产出
     * 「两臂其实同一条路径」的假对比。
     */
    async assertEngaged(baseUrl) {
      ...
      if (!res || !res.ok || !body.includes('class="home"')) {
        throw new Error(
          [
            `「vite」臂未生效：${baseUrl}/ 未返回 SSR 页面（status=${res?.status ?? 'n/a'}）。`,
            '',
            '该臂要求插件接管 dev（请求路由 + 自举宿主 app）。',
    ...
    /**
     * build 臂的同一证明：`vp build` 没有 CLI，服务端产物只可能来自插件注册的
     * `builder.buildApp` —— 实测不带接线时它连构建都跑不起来（`Cannot resolve entry module
     * index.html`，exit 1，零产物）。这里的产物契约断言是第二道防线。
     */
    assertBuildEngaged(distDir, violations) {
      ...
      if (countHtmlFiles(distDir) === 0) violations.push('没有预渲染 HTML');
    }
  }
};
...
async function main() {
  ...
  if (!skipBuild && armNames.length > 1) {
    console.log(
      '注：两臂的命令不同（`pnpm exec ubean …` vs `pnpm exec vp …`）。ADR-0012 收敛后它们共用\n' +
        '    同一套生命周期，因此差值反映的是**入口开销**（CLI 的急切 import 图 vs vite 启动），\n' +
        '    不是两种实现的性能差 —— 详见 ARMS 里的说明。'
    );
    console.log('');
  }
